...
def fetch_salary_cap_csv_path():
    return '../../Data/Salary/SALARY_CAP.csv'
...
def fetch_lower_salary_cap(season):
    ...
    next_season_suffix = str(season + 1)[-2:]
    key_season = f"{season_str}-{next_season_suffix}"

    matching_row = df_salary_cap[df_salary_cap['SEASON'] == key_season]
    if not matching_row.empty:
        return convert_currency_to_int(matching_row['LOWER LIMIT'].iloc[0])
    else:
        return None
...
def main():
    for season in range(2008, 2025):
        ...
        lower_salary_cap = fetch_lower_salary_cap(season)
        df_season['Y_Salary_Cap_Percentage'] = df_season['CAP HIT'].apply(convert_currency_to_int) / lower_salary_cap
